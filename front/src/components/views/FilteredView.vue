<template>
    <div class="event-list">
      ...
      <!-- Кнопка для показа следующей строки -->
      <button v-if="visibleEvents.length < events.length" @click="showMore" class="load-more-button">
        Показать еще
      </button>
    </div>
  </template>
  ...
  export default {
    data() {
      return {
        ...
        perPage: 6,
        visibleCount: 6, // Сколько карточек видно вначале
      };
    },
    computed: {
      visibleEvents() {
